...
[work in progress](conventions.md)
...
* `Моноширинный шрифт` применяется для имен команд и их параметров, а также
  для примеров выполнения и вывода команд.
...
* `--volume VOLUME_NAME:/persist`
  ...
  (см. [Подготовка контейнера]). `/persist` - папка в контейнере, где
  будет cмонтирован том.
* `--restart=always`
...
Номер порта на локальной системе может принимать значения от 2048 до 65535.
...
Этот же путь следует указать в системе **АРМ С3000**: поле «Устройство»
