...
Запуск производится командой `docker run` со следующими параметрами:
* `--name arm-s3000`
...
* `arm-s3000-astra-smolensk_1.7:VERSION`
  Имя образа `Docker`. *VERSION* в имени файла, как и ранее, следует заменить
  на номер версии образа, с которым фактически происходит работа.

После успешного запуска контейнера соединение с системой **АРМ С3000**
возможно на всех сетевых интерфейсах, например:
...
1. При подключении приборов к **АРМ С3000** через **С2000-Ethernet**
