...
В **s3k** реализована возможность рассылки уведомлений через
мессенджер Telegram о событиях, происходящих на управляемом объекте,
с помощью специальных чат-ботов Telegram.

Для использования этой возможности необходимо наличие
подключения к сети Интернет и учетная запись Telegram.

События, включаемые в состав уведомлений,
определяются "каналом уведомлений" (КУ) - определенным набором фильтров.
