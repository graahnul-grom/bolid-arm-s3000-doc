...
Способы подключения устройств ИСО "Орион" к компьютеру с **s3k**:
1. через последовательный порт RS-232
2. через порт RS-485
3. по сети, через устройство **s2k-eth**

Приборы могут подключаются как напрямую, так и через преобразователи интерфейсов.
(`2DO` some bolid hw converters examples here, with names)

Каждому из трех способов подключения в **s3k**
ставится в соответствие понятие *порт*.

соответствует понятие *порт*
именуются *портом*

Каждый из трех способов подключения именуются *портом*,
вне зависимости от того, идет речь о физическом порте
или о сетевом подключении посредством **s2k-eth**.

идет речь о сетевом подключении посредством **s2k-eth**
или же имеется в виду физический порт
...
Преобразователи чаще всего применяются с устройствами RS-232 и
RS-485, например, в USB - для удобства подключения к компьютеру.

Настройка производится посредством добавления *порта* - `RS` или
`С2000-Ethernet` на странице
