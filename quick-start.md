...
- **Примечание:**
  так обозначены краткие аннотации к основному тексту.
<!--
-*Курсив* применяется для обозначения технических терминовю
  а также в других случаях для выделений в тексте.
  и, в общем случае, для выделений частей текста.
-->
...
-->
  ...
  [см. "Параметры второго порта связи" в руководстве по эксплуатации **s2km**]()
...
- `ORIG` `BUG` Режим работы - "Компьютер"
  не *Режим работы* (s2km manual, p.15),
  - дежурный
  - пуск
  - пожар
  - etc...
  а  *Параметры второго порта связи -> Режим* (p.109)
  - ПРИНТЕР
  - КОМПЬЮТЕР
  - RS-485 РЕЗЕРВ
  - ПИ/РЕЗЕРВ
...
- `2ASK` how does it relate to **s3k**:
  ...
  *Параметры второго порта связи -> СКОРОСТЬ* (p.110)
-->
...
Для отображения состояния приборов и зон, задействованных
в ИСО "Орион" и управления ими в **s3k** требуется
доступ к конфигурации пульта.

Создание такой конфигурации (с возможностью экспорта в файл)
...
Работа с конфигурациями начинается с сохранения пульта
в базе данных **s3k**.

На странице
*`Конфигурирование структуры объекта`* → *`Приборы`* → *`Линии и протоколы`*
необходимо выбрать нужную линию, к которой подключен пульт,
зайти в состав линии и выбрать устройство пульта.
В правой части окна нажать "".
...
- TCP/IP
- ip/port of **s3k**
- login as a user with "services" role
- select needed s2km
