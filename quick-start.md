...
После чего следует включить параметр *`Общая настройка`* →
Также следует включить параметр *`Общая настройка`* →
*`Сервисное обслуживание`* → *`Прочие настройки`* →
*`Доступ к устройству по протоколу https`*, после чего
система готова к приему соединений по протоколу HTTPS.

- **Внимание:**<br />
При изменении большинства параметров на странице настройки, система **s3k**
...
# Подключение устройств
- `2DO` `2ASK` "Line" definition (protocol + port)
```
